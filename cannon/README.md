...
```shell
# Build bl-program server-mode and MIPS-client binaries.
cd ../bl-program
make bl-program # build

# Switch back to cannon, and build the CLI
cd ../cannon
make cannon

# Transform MIPS bl-program client binary into first VM state.
# This outputs state.json (VM state) and meta.json (for debug symbols).
./bin/cannon load-elf --path=../bl-program/bin/bl-program-client.elf

# Run cannon emulator (with example inputs)
# Note that the server-mode bl-program command is passed into cannon (after the --),
