...
func (n *OpNode) handleProtocolVersionsUpdate(ctx context.Context) error {
	...
	if err != nil {
		...
		catalyst.LogProtocolVersionSupport(n.log.New("node", "bl-node"), engineSupport, recommended, "recommended")
		catalyst.LogProtocolVersionSupport(n.log.New("node", "bl-node"), engineSupport, required, "required")
	}
