...
```shell
cd bl-node
go build -o bin/bl-node ./cmd
```

## Testing

Run bl-node unit tests:
```shell
cd bl-node
go test ./...
```
...
The `bl-node` can generate geth compatible `genesis.json` files. These files
...
that exist in the state and act as system level contracts. The `bl-node` can
...
supported by the `bl-node`. The network name must match the name of the file
in the deploy config directory.

Example usage:

```bash
$ bl-node genesis devnet-l2 \
