...
def main():
    ...
    op_node_dir = pjoin(args.monorepo_dir, 'bl-node')
    ...
    ops_chain_ops = pjoin(monorepo_dir, 'bl-chain-ops')
...
def devnet_deploy(paths):
    ...
    log.info('Bringing up `bl-node`, `bl-proposer` and `bl-batcher`.')
    run_command(['docker', 'compose', 'up', '-d', 'bl-node', 'bl-proposer', 'bl-batcher'], cwd=paths.ops_bedrock_dir, env={
        'PWD': paths.ops_bedrock_dir,
