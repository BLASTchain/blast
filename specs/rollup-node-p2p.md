...
The bl-node implements a similar request-response encoding for its sync protocols as the L1 ethereum Beacon-Chain.
...
The req-resp protocols served by the bl-node all have `/ProtocolPrefix` set to `/opstack/req`.
...
- `/MessageName` is `/block_by_number/<chain-id>` where `<chain-id>` is set to the bl-node L2 chain ID.
- `/SchemaVersion` is `/0`
