...
with a bond at stake if the proof is wrong. The [bl-proposer](../bl-proposer/) in one such implementation of a proposer.
...
The [L2 output proposer](../bl-proposer) is expected to submit output roots on a deterministic
...
The honest `bl-proposer` algorithm assumes a connection to the `L2OutputOracle` contract to know
the L2 block number that corresponds to the next output proposal that must be submitted. It also
assumes a connection to an `bl-node` to be able to query the `optimism_syncStatus` RPC endpoint.
