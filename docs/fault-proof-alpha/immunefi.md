...
The current system is not production ready, however the core infrastructure for creating an instruction trace ([Cannon][cannon] + the [`bl-program`][bl-program]), the off-chain challenge agent ([`bl-challenger`][bl-challenger]),
...
1. Limitations of pre-image oracle inputs. The pre-image oracle currently does not support the full specified set of inputs.
    ...
    The remaining pre-images types are not supported, as the types are not used by the current bl-program, but may be supported for future program proving, e.g. type 3 for application-specific proofs, and new types 4, 5, etc. for ethereum extensions like SHA2 and KZG point verification.
...
### Reviewer Notes
1. **Any bug report without a proof-of-concept in the form of a test in `bl-e2e` will not be considered a valid bug report.**
...
1. **bl-program**: The [`bl-program`][bl-program] binary and its dependencies.
1. **Smart Contracts**
    1. The [Cannon][cannon-contracts] contracts and their dependencies.
    1. The [Dispute Game][dispute-game] and their dependencies.
1. **bl-challenger**: The [`bl-challenger`][bl-challenger] binary and its dependencies.
...
as well as the [`bl-challenger`][bl-challenger] in order to support the features that will bring the system to a production ready state. During this time, it is unlikely that [Cannon][cannon], the [Cannon contracts][cannon-contracts],
or the [`bl-program`][bl-program] will change significantly, and as such, we recommend focusing efforts primarily on these components.
...
1. **Cannon**
    1. [Cannon][cannon]'s `mipsevm` must be functionally equivalent to the [MIPS thread context][cannon-contracts] implemented in Solidity. Any disparities that result in different `bl-program` execution are a bug.
...
1. **bl-challenger**
    1. The honest `bl-challenger` must never make a claim that does not support the honest outcome of the dispute game (i.e., the outcome which favors the honest L2 state being considered canonical).
...
Bug reports in the form of a proof-of-concept `bl-e2e` test that demonstrates a violation of any of the above invariants will be considered valid bug reports and eligible for a reward*.

* All proof of concept reports should be configured to run against the parameters of the system deployed on the `goerli` testnet or with the environment defined in the `bl-e2e` `faultproof_test.go` file. Bug reports that otherwise violate the above invariants
...
<!-- LINKS -->
[cannon]: https://github.com/BLASTchain/blast/tree/develop/cannon
[cannon-vm-specs]: https://github.com/BLASTchain/blast/blob/develop/specs/cannon-fault-proof-vm.md
[dispute-game]: https://github.com/BLASTchain/blast/tree/develop/packages/contracts-bedrock/src/dispute
[fault-dispute-specs]: https://github.com/BLASTchain/blast/blob/develop/specs/fault-dispute-game.md
[cannon-contracts]: https://github.com/BLASTchain/blast/tree/develop/packages/contracts-bedrock/src/cannon
[bl-program]: https://github.com/BLASTchain/blast/tree/develop/bl-program
[bl-challenger]: https://github.com/BLASTchain/blast/tree/develop/bl-challenger
[alphabet-vm]: https://github.com/BLASTchain/blast/blob/c1cbacef0097c28f999e3655200e6bd0d4dba9f2/packages/contracts-bedrock/test/FaultDisputeGame.t.sol#L977-L1005
[fault-proof-specs]: https://github.com/BLASTchain/blast/blob/develop/specs/fault-proof.md
[immunefi]: https://immunefi.com/bounty/optimism/
[invalid-proposal-doc]: https://github.com/BLASTchain/blast/blob/develop/docs/fault-proof-alpha/invalid-proposals.md
