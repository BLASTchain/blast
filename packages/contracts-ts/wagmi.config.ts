 ...
 * Keep this in sync with bl-bindings/predeploys/addresses.go in meantime
 */
...
const deployments = {
  [1]: glob.sync(
    `node_modules/@eth-blast/contracts-bedrock/deployments/${chains[1]}/*.json`
  ),
  [10]: glob.sync(
    `node_modules/@eth-blast/contracts-bedrock/deployments/${chains[10]}/*.json`
  ),
  [5]: glob.sync(
    `node_modules/@eth-blast/contracts-bedrock/deployments/${chains[5]}/*.json`
  ),
  [420]: glob.sync(
    `node_modules/@eth-blast/contracts-bedrock/deployments/${chains[420]}/*.json`
  ),
}
