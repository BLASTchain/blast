...
contract SystemConfig is OwnableUpgradeable, ISemver {
    ...
    ///         layout from the way that `solc` lays out storage. The `bl-node` uses a storage
    ...
    /// @notice The block at which the bl-node can start searching for logs from.
    ...
    /// @param _startBlock        Starting block for the bl-node to search for logs from.
    ...
    /// @param _batchInbox        Batch inbox address. An identifier for the bl-node to find
    ///                           canonical data.
    ...
    ///         In the future, this method may emit an event that the `bl-node` picks up
