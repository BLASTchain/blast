...
func TestCannonServer(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		cfg := configForArgs(t, addRequiredArgsExcept(config.TraceTypeCannon, "--cannon-server", "--cannon-server=./bl-program"))
		require.Equal(t, "./bl-program", cfg.CannonServer)
	})
}
...
func runWithArgs(cliArgs []string) (log.Logger, config.Config, error) {
	...
	fullArgs := append([]string{"bl-challenger"}, cliArgs...)
