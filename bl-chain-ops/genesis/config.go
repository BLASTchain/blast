...
import (
	...
	"github.com/BLASTchain/blast/bl-bindings/hardhat"
...
// https://github.com/BLASTchain/blast/blob/develop/packages/contracts-bedrock/src/libraries/Constants.sol
...
type DeployConfig struct {
	...
	// SystemConfigStartBlock represents the block at which the bl-node should start syncing
	...
	// TODO(clabby): Right now, the build of the `bl-program` is nondeterministic, meaning that
	// the binary must be distributed in order for honest actors to agree. In the future, we'll
	// look to make the build deterministic so that users may build Cannon / the `bl-program`
	// from source.
